async function loadSummaryStrategi(intahun) {
  const res = await fetch(`/api/summary_strategi?tahun=${intahun}`);
...
async function loadRangkumanStrategi(intahun) {
  const res = await fetch(`/api/rangkuman_strategi?tahun=${intahun}`);
  const data = await res.json();
  ...
  data.forEach(d => {
    ...
    tr.innerHTML = `
      ...
      <td style="text-align:center">${persentaseSub.toString().replace('.', ',')}%</td>
      <td style="text-align:center">${d.total_anggaran.toLocaleString('id-ID')}</td>
      <td style="text-align:center">${persentaseAnggaran.toString().replace('.', ',')}%</td>
      ...
      <td style="text-align:center">${d.persentase_realisasi.toString().replace('.', ',')}%</td>
    `;
  ...
  trTotal.innerHTML = `
    ...
    <td style="text-align:center"><strong>${persentaseTotalRealisasi.toString().replace('.', ',')}%</strong></td>
  `;
  tbody.appendChild(trTotal);
}

async function loadDetailSubkegiatan(intahun) {
  const res = await fetch(`/api/detail_subkegiatan?tahun=${intahun}`);
  ...
  summary.forEach(s => {
    ...
    trMain.innerHTML = `
      <td colspan="10"><strong>${s.strategi}</strong> 
        ...
        Persentase: ${s.persentase_realisasi.toString().replace('.', ',')}%)
...
async function loadYears() {
  ...
  // pilih default tahun (misalnya tahun terakhir)
  const defaultYear = years[years.length - 1];
  select.value = defaultYear;

  // panggil grafik pertama kali
  loadSummaryStrategi(defaultYear);
  loadRangkumanStrategi(defaultYear);
  loadDetailSubkegiatan(defaultYear);

  // pasang event listener untuk update kalau dropdown berubah
  select.addEventListener("change", (e) => {
    const tahun = e.target.value;
    loadSummaryStrategi(tahun);
    loadRangkumanStrategi(tahun);
    loadDetailSubkegiatan(tahun);
  });
}
